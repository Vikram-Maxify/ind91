...
$('.start-order').click(function (e) {
    ...
    if (value !== '') {
        let valuesArray = value.split(',').map(v => v.trim()); // Convert to an array and trim spaces
        let uniqueValues = new Set(valuesArray); // Create a Set to remove duplicates

        if (uniqueValues.size !== valuesArray.length) { 
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'All values must be unique!',
            });
